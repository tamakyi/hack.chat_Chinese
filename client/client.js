...
var frontpage = [
	...
	"",
        "***管理员不叫admin哦***",
        "这是一个开源的匿名网页聊天软件，不需要注册，即开即用",
        "创建聊天室:浏览器访问https://chat.tamakyi.club/?test 即可",
        "？后面即为聊天室名称，仅支持英文和数字组合",
        "*注意1:本站不存储任何数据，页面刷新则数据丢失!*",
        "*注意2:因为cloudflare的 原因，首次加载时会有些缓慢，请耐心等待.*",
	"以下是一些默认的聊天室::",
	"?lounge ?meta",
	"?math ?physics ?chemistry",
	"?technology ?programming",
	"?games ?banana",
	"然后这是一个随机生成的聊天室: ?" + Math.random().toString(36).substr(2, 8),
	"",
	"格式:(以下为机翻)",
	"保留空白，因此源代码可以按原样粘贴.",
	"支持LaTeX风格,需要在句首添加一个美元符号来区别 $\\zeta(2) = \\pi^2/6$, 两个美元符号则用于显示. $$\\int_0^1 \\int_0^1 \\frac{1}{1-xy} dx dy = \\frac{\\pi^2}{6}$$",
	"对于语法突出显示，请将代码包装为: ```<language> <the code>```  <language> 内为我们所知的程序语言.",
	"",
	"作者现在的此项目 Github: https://github.com/hack-chat",
	"作者原来的此项目 GitHub: https://github.com/AndrewBelt/hack.chat",
	"",
	"Bots, Android clients, desktop clients, browser extensions, docker images, programming libraries, server modules 和其他第三方插件:",
	"https://github.com/hack-chat/3rd-party-software-list",
	"",
	"在WTFPL和MIT开源许可下允许发布服务器和web客户端。.",
	"程序不保留任何历史数据."
].join("\n");
...
function join(channel) {
	...
	ws.onclose = function () {
		if (wasConnected) {
			pushMessage({ nick: '!', text: "与Socket的连接丢失，尝试重连中. . ." });
		}
